...
The repository also builds for [F-Droid](https://f-droid.org) — the Android target builds
from a bare clone with no manual steps, and the store listing lives in
`fastlane/metadata/android/` — see [`fdroid/README.md`](./fdroid/README.md) for the
packaging recipe and how to submit it.
...
A fresh clone builds as it is — `./gradlew :desktop:run`, `./gradlew :android:assembleRelease` —
because everything the app loads is either in the repository or generated by the build:

* The interface icons in `./assets/icons/` are rendered from the SVG masters in
  `./assets_nonshared/icons/` by `./gradlew :core:generateIcons` (Batik, pure Java, so no
  librsvg or ImageMagick needed), and the Android launcher mipmaps by
  `:android:generateLauncherIcons`. Both run automatically before anything packages the
  assets; run them by hand after editing an SVG or a logo master. The PNGs themselves are
  not in git.
* The font the app renders with, Liberation Sans Regular 2.1.5, is in
  `./assets/liberation_fonts/` with its
  [SIL OFL 1.1](./assets/liberation_fonts/LICENSE) licence.

One thing is optional and *not* generated by a plain build: the offline place-search index.
Without it the app still builds and runs, and searching a place uses OpenStreetMap's
Nominatim online instead. To ship offline search too:
...
Then build with Gradle (the wrapper, `./gradlew`; Gradle 9.3.0 and JDK 17 — see
[AGENTS.md](./AGENTS.md)); Android Studio opens the project as it is.
