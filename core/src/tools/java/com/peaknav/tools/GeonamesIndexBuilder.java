 ...
 *
 * <p>Run it through Gradle:
 *
 * <pre>
 * ./gradlew :core:buildGeonamesIndex --args="cities500.txt alternateNamesV2.txt assets/geonames_index.362"
 * </pre>
 *
 * <p>It lives in its own {@code tools} source set: it is a build-time data tool, not part of the
 * app, so the index-writing half of Lucene never reaches the APK. It used to sit inside
 * TestLuceneGeonames, where a half-built index directory left behind by a previous run could fail
 * the test suite.
 *
 ...
 * languages). Point {@code CITIES_FILE} and {@code ALTERNATE_NAMES_FILE} at them and run
 * {@code main}.
 *
...
public class GeonamesIndexBuilder {
    ...
    public static final String CITIES_FILE = "cities500.txt";
    public static final String ALTERNATE_NAMES_FILE = "alternateNamesV2.txt";
    public static final String INDEX_DIR = "geonames_index.362";
     ...
     * {@code selectNames}), so the cap drops the least useful ones — without it a handful of
    ...
    public static final class Place {
        ...
        public final List<String> preferred = new ArrayList<>(2);
        /** Names in the app's languages, kept when there is room. */
        public final List<String> translated = new ArrayList<>(4);
    }
     ...
     * </ol>
     */
    public static List<String> selectNames(Place p) {
    ...
    public static String stripAccents(String s) {
        ...
        for (int i = 0; i < n.length(); i++) {
            ...
            // Letters NFD cannot reduce: they are not accented letters but letters of
            // their own, with a conventional ASCII spelling. Without these, Großglockner
            // never gains a "grossglockner" alias and an ASCII keyboard reaches it only
            // through fuzzy matching - which ranks it below any exact match that a
            // namesake village happens to have.
            switch (ch) {
                case 'ß': sb.append("ss"); break;
                case 'ẞ': sb.append("SS"); break;
                case 'ø': sb.append('o'); break;
                case 'Ø': sb.append('O'); break;
                case 'đ': case 'ð': sb.append('d'); break;
                case 'Đ': case 'Ð': sb.append('D'); break;
     ...
     */
    public static float populationBoost(String population) {
        long pop;
        try {
