...
public class LuceneAssetLoader {
    ...
    /**
     * The searcher over the packaged index, or {@code null} when the build carries no index: it
     * is built separately from the GeoNames dumps, and a build made without it - F-Droid's, for
     * one - searches online only. Not an error to throw: this runs on a background thread, and
     * on Android an uncaught exception on any thread kills the whole process.
     */
